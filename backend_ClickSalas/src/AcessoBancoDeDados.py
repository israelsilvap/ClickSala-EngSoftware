...
def get_turma_names():
    ...
    turma_names = [row[0] for row in cursor.fetchall()]
    cursor.close()
    return turma_names
...
def get_salas_names():
    ...
    cursor.execute(command)
    # Armazenar todos os nomes de turmas em uma lista
    salas_names = [row[0] for row in cursor.fetchall()]
    cursor.close()
    return salas_names

def create_turma_from_db():
    cursor = conexao.cursor()
    turma_names = get_turma_names()
    turmas = []
...
def create_sala_from_db():
    cursor = conexao.cursor()
    salas_names = get_salas_names()
    salas = []
